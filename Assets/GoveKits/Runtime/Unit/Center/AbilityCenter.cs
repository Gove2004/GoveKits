...
namespace GoveKits.Runtime.Unit
{
	...
	/// </summary>
	/// <remarks>
	/// 仅扫描并注册带 <see cref="AutoUnitAttribute"/> 的 UnitAbility 派生类。
	/// </remarks>
	public static class AbilityCenter
	{
		private static readonly Dictionary<Type, Func<IUnit, object[], UnitAbility>> _factories = new();
		...
		public static void ScanAndRegisterFactories()
		{
			...
			_factories.Clear();
			var abilityBaseType = typeof(UnitAbility);
			foreach (var type in GetAllLoadableTypes())
			{
				if (type == null || type.IsAbstract || !abilityBaseType.IsAssignableFrom(type))
				{
					continue;
				}
		...
		/// 按类型创建 Ability。
		/// </summary>
		public static UnitAbility Create(Type abilityType, IUnit owner, params object[] args)
		{
			if (abilityType == null)
			{
		...
		/// 按泛型类型创建 Ability。
		/// </summary>
		public static TAbility Create<TAbility>(IUnit owner, params object[] args) where TAbility : UnitAbility
		{
			return (TAbility)Create(typeof(TAbility), owner, args);
		}
		...
		private static Func<IUnit, object[], UnitAbility> BuildFactory(Type abilityType)
		{
			var constructors = abilityType.GetConstructors(BindingFlags.Public | BindingFlags.Instance);
			if (constructors.Length == 0)
			{
				throw new InvalidOperationException($"Ability type {abilityType.FullName} has no public constructor.");
			}
			...
			return (owner, args) =>
			{
				...
				{
					throw new InvalidOperationException(
						$"No matching constructor found for {abilityType.FullName}. " +
				...
				return (UnitAbility)match.Invoke(invokeArgs);
			};
		}
