...
namespace GoveKits.Runtime.Unit
{
    ...
    public class AttributeContainer : IUnitTagSource, IEnumerable<KeyValuePair<UnitTag, UnitAttribute>>
    {
        ...
        public RuntimeAttribute AddRuntime(UnitTag name, UnitTag sourceTag)
        {
            var sourceAttr = GetState(sourceTag);
            if (sourceAttr == null)
            {
                throw new KeyNotFoundException($"无法创建运行时属性 {name}，因为未找到对应的状态属性 {sourceTag}。");
            }
